...
| `transitions-dev-repo` | `npx skills add`, `npx transitions-refine live`, agent-backed refine/apply commands | Not executed; workflow analyzed only | Running third-party packages was unnecessary for inspection and would cross an execution boundary before task-specific dependency review. Tool ideas were evaluated from source pages/repository inventory. |
