...
| `anthropic-agent-skills` | Apache-2.0 for the consulted `frontend-design`, `webapp-testing`, and `web-artifacts-builder` skills at `fa0fa64bdc967915dc8399e803be67759e1e62b8`; other repository skill folders can carry different terms | `summarized`; consulted skill text/code `adaptable-with-notice` | Preserve Apache-2.0, copyright, attribution, NOTICE, and modified-file obligations for substantial copies or derivatives. This corpus uses original synthesis only. Do not infer rights for Anthropic marks, preset assets, third-party components, or separately licensed document skills. |
...
| `kill-ai-slop` | Unstated; GitHub exposed no root license or repository license declaration at refreshed revision `2d21245d` (2026-07-17) | `link-only` / `inspiration-only`; copying and adaptation `unresolved` | Credible as primary evidence for its own active practitioner project, but that does not grant reuse rights. Use generalized observations only; do not copy or adapt its prose, examples, detection regexes, scanner, or skill text unless the license is clarified. |
...
The 395 entries in `source-discovery/seed-catalog.json` are discovery metadata, not accepted license evidence. Their inherited default is “unverified; do not copy or adapt.” Explicit gallery families are `inspiration-only`; other unreviewed seeds are `unresolved`. A visible component, copy button, npm command, public HTML response, “free” label, paid subscription, or account entitlement does not establish redistribution rights.

Before moving any seed into `source-registry.json`, review the exact code/docs/template/asset license; ownership; attribution/notice/share-alike duties; paid or account boundaries; fonts/icons/images/marks; dependency licenses; and intended use. OpenAI Build Week and similar corporate/product/event marketing pages are never pullable catalogs and do not enter this review unless the user supplied one as a case study for a specific task.
